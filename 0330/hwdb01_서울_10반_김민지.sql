...
select ename, empno, sal, round(12*sal + sal*1.5) as yearsal
from emp
where deptno = 30;

-- 5) 급여가 $2,000 이상인 모든 사람은 급여의 15%를 경조비로 내기 로 하였다. 이름, 급여, 경조비를 검색하세요.
select ename, sal, round(sal*0.15) as fee
from emp
where sal >= 2000;
